  ...
  try {
    ...
    if (!data.length) {
      document.getElementById("weight-stats").innerHTML =
        '<p class="muted">No weight data yet.</p>';
      return;
    }
    ...
    document.getElementById("weight-stats").innerHTML = `
      <div class="stat">
        ...
        <strong>${formatNumber(latest, 1)} kg</strong>
      </div>
      <div class="stat">
        <div>First logged</div>
        <strong>${formatNumber(first, 1)} kg</strong>
      </div>
      <div class="stat">
        <div>Change</div>
        <strong>${formatNumber(diff, 1)} kg</strong>
      </div>
      <div class="stat">
    ...
    document.getElementById("weight-stats").innerHTML =
      '<p class="muted">Unable to load chart.</p>';
  }
}
