...
async function loadGlucose() {
  ...
  try {
    ...
    const preferredUnit = data.preferred_unit;
    const formatted = formatGlucoseDisplay(reading.value_mgdl, preferredUnit);
    valueEl.textContent = formatted.valueText !== '—' ? `${formatted.valueText} ${formatted.unitLabel}` : '—';
...
function scheduleNextGlucoseRefresh(lastTimestamp) {
  ...
  if (lastTimestamp) {
    const age = Math.abs(Date.now() - lastTimestamp);
    const withinHour = age < 60 * 60 * 1000;
    if (withinHour) {
      const targetTime = lastTimestamp + GLUCOSE_INTERVAL_MS;
      const untilTarget = targetTime - Date.now();
      delay = Math.min(GLUCOSE_INTERVAL_MS, Math.max(30 * 1000, untilTarget));
    }
  }
  glucoseRefreshTimer = setTimeout(() => loadGlucose(), delay + 5 * 1000);
}
