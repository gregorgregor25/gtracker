...
const GLUCOSE_UNIT_MGDL = 'mgdl';
const GLUCOSE_UNIT_MMOL = 'mmol';
...
function normalizePreferredUnit(unit) {
  if (!unit) return GLUCOSE_UNIT_MGDL;
  const value = String(unit).toLowerCase();
  if (value.includes('mmol')) return GLUCOSE_UNIT_MMOL;
  if (value.includes('mg')) return GLUCOSE_UNIT_MGDL;
  return GLUCOSE_UNIT_MGDL;
}

function formatGlucoseDisplay(valueMgdl, preferredUnit) {
  const unitToken = normalizePreferredUnit(preferredUnit);
  const label = unitToken === GLUCOSE_UNIT_MMOL ? 'mmol/L' : 'mg/dL';
  if (valueMgdl === null || valueMgdl === undefined || Number.isNaN(Number(valueMgdl))) {
    return { valueText: '—', unitLabel: label, numeric: null };
  }
  const mgValue = Number(valueMgdl);
  if (unitToken === GLUCOSE_UNIT_MMOL) {
    const mmol = mgValue / 18;
    return { valueText: mmol.toFixed(1), unitLabel: label, numeric: mmol };
  }
  return { valueText: Math.round(mgValue).toString(), unitLabel: label, numeric: Math.round(mgValue) };
}
...
function buildBadgeCatalog(entries = [], summary = {}, streaks = {}) {
  ...
  const totalTreadmillMinutes = sorted.reduce((sum, e) => sum + (Number(e.treadmill_minutes) || 0), 0);
  const totalTreadmillDistance = sorted.reduce((sum, e) => sum + (Number(e.treadmill_distance_km) || 0), 0);
  const treadmillGoalDays = sorted.filter((e) => (Number(e.treadmill_minutes) || 0) >= TREADMILL_GOAL_MINUTES).length;
  const gymDays = sorted.filter((e) => e.gym_done).length;

  const firstWeight = sorted.find((e) => e.weight_kg !== null && e.weight_kg !== undefined)?.weight_kg;
  const latestWeight = [...sorted].reverse().find((e) => e.weight_kg !== null && e.weight_kg !== undefined)?.weight_kg;
  const weightDelta = firstWeight !== undefined && latestWeight !== undefined ? firstWeight - latestWeight : 0;

  const weeklyConsistency = summary?.consistency_score ?? 0;
  const loggedDaysThisWeek = (summary?.entries || []).filter((e) => e && e.date).length;
