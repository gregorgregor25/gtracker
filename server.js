...
async function upsertEntry(entry) {
  ...
  if (existing) {
    await run(
      `UPDATE entries SET gym_done=?, treadmill_minutes=?, treadmill_distance_km=?, calories_gym=?, calories_treadmill=?, calories_total=?, calories_burned=?, carbs=?, weight_kg=?, mood=?, notes=? WHERE date=?`,
      [
        entry.gym_done,
  ...
  const result = await run(
    `INSERT INTO entries (date, gym_done, treadmill_minutes, treadmill_distance_km, calories_gym, calories_treadmill, calories_total, calories_burned, carbs, weight_kg, mood, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      entry.date,
      entry.gym_done,
      entry.treadmill_minutes,
...
app.get('/api/summary/daily-goal', async (_req, res) => {
  try {
    ...
    const todayEntry = await get('SELECT calories_total, calories_burned, calories_gym, calories_treadmill FROM entries WHERE date = ?', [todayString()]);
