...
app.post('/api/glucose/config', (req, res) => {
  try {
    ...
    // Save credentials AND unit preference
    setCredentials({ email, password, region, tld, unit });

    // Explicitly update preferred unit
    if (unit) {
    ...
    const status = getCredentialStatus();
    status.unit = unit || status.unit;

    res.json({ ok: true, ...status });

  } catch (err) {
    console.error('Failed to save glucose config:', err);
    res.status(500).json({
      ok: false,
      error: 'Unable to save LibreLinkUp credentials'
    });
  }
});
